...
- Objective: 把首版快升级版明确拆成 `M17-M21` 里程碑顺序：`M17 daemon core -> M18 VS Code host -> M19 resume bridge -> M20 validation -> M21 rollout resume`
...
- [x] EL-4 define the `M17-M21` milestone order around daemon + host bridge, and record it in roadmap / plan for user-directed execution
...
- Slice: define-m17-through-m21-daemon-host-mainline
  - Objective: 把快升级版正式拆成 `M17-M21` 里程碑主线，并明确首个宿主是 VS Code 扩展前端
  ...
  - Validation: roadmap / development plan / status / plan 都能直接读出相同的 `M17-M21` 顺序
